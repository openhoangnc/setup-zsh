...
- **Dọn dẹp Mac (`cdm`)** — Cài đặt [**CleanDevMac**](https://github.com/cleandevmac/cdm), công cụ dọn dẹp độc lập của tôi (vốn ra đời từ chính repo này). Nó thu hồi dung lượng đĩa từ các cache lập trình và sản phẩm build có thể tạo lại (Xcode, Go, Node/npm/pnpm/yarn, Gradle, Maven, Cargo, Python, Homebrew, Playwright, và nhiều hơn nữa), xóa cache của Electron/trình duyệt/ứng dụng, quét dọn rác dự án nhóm theo từng repo, thu hồi dung lượng Docker/Podman, và phát hiện dữ liệu còn sót lại từ các ứng dụng bạn đã gỡ cài đặt — không bao giờ xóa bất cứ thứ gì khi bạn chưa xác nhận kế hoạch chi tiết.
...
Việc dọn dẹp giờ do [**CleanDevMac**](https://github.com/cleandevmac/cdm) đảm nhiệm — công cụ độc lập của tôi, vốn ra đời từ repo này và nay có repo riêng. Khi cài đặt, bản phát hành mới nhất được tải về `~/.zsh/setup-zsh/bin/cdm`; chạy bằng lệnh `cdm` (tên cũ `clean-my-mac` vẫn dùng được).

[![CleanDevMac](https://raw.githubusercontent.com/cleandevmac/cdm/main/screenshot.png)](https://github.com/cleandevmac/cdm)

Công cụ mở một menu tương tác: quét máy Mac của bạn, nhóm mọi thứ có thể thu hồi an toàn thành các danh mục sắp xếp từ lớn đến nhỏ, và chỉ xóa những mục bạn đánh dấu — cache lập trình và sản phẩm build, cache của Electron/trình duyệt/ứng dụng, rác dự án nhóm theo từng repo, Docker/Podman, và dữ liệu còn sót lại của các ứng dụng bạn đã gỡ. Bạn luôn thấy kế hoạch chi tiết trước khi bất cứ thứ gì bị xóa.
...
Các phím tắt TUI, danh sách đầy đủ những gì được dọn, các bảo đảm an toàn, và tệp JSON quy tắc có thể chỉnh sửa đều được ghi trong [README của CleanDevMac](https://github.com/cleandevmac/cdm#readme).
