...
- **Tự động gợi ý so khớp bất kỳ (Match-Any Autosuggestions)**: Tìm kiếm trong toàn bộ lịch sử của bạn để gợi ý bất kỳ lệnh nào chứa nội dung bạn đã nhập (ví dụ: gõ `google` sẽ tự động gợi ý và khớp lệnh `curl -I google.com`).
