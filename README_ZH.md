...
- **导航**：用 **上 / 下 方向键**（或 `j` / `k`）移动光标（`❯`）。类别按占用空间从大到小排列，收益最大的排在最前面。
- **选择**：按 **空格键** 勾选/取消某个类别（`[ ]` ↔ `[✓]`）。安全、可再生的缓存已默认勾选；风险较高的项目（Maven 仓库、Playwright、崩溃日志）、项目文件夹以及残留的应用数据默认**不勾选**。
- **详情**：按 **回车**（或 **`d`**）查看当前高亮类别内的具体路径和大小；按任意键返回。
